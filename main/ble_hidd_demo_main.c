...
static void hidd_event_callback(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param)
{
   ...
   // xQueueReceiveFromISR(xQueueWheelHandle,wheel_value,pxTaskWoken)
    return;
    ...
    if(device_mode == mouse_mode){
        switch (dat.key)
        {
            case 0: //左键
                ...
                break;
            case 1: //上
                esp_hidd_send_mouse_value(hid_conn_id,0,0,-50,0);break;
            case 2: //下
                esp_hidd_send_mouse_value(hid_conn_id,0,0,50,0);break;
            case 3: //左
        ...
        switch (dat.key)
        {
            case 0:
                off_on = ~off_on;
                if(off_on == mouse_mode) {
                    //esp_hidd_send_keyboard_value(hid_conn_id, 0,HID_KEY_PAUSE , 1);
                    //esp_hidd_send_consumer_value(hid_conn_id, HID_CONSUMER_PLAY  , true);
                    //vTaskDelay(1);
                   // esp_hidd_send_consumer_value(hid_conn_id, HID_CONSUMER_PLAY ,false);
                }
                else{
                     //esp_hidd_send_keyboard_value(hid_conn_id, 0,HID_KEY_INSERT , 1);
            ...
            case 2:
                ...
                break;
            case 3:
                esp_hidd_send_consumer_value(hid_conn_id, HID_CONSUMER_SCAN_PREV_TRK, true);
                vTaskDelay(1);
                esp_hidd_send_consumer_value(hid_conn_id, HID_CONSUMER_SCAN_PREV_TRK, false);
                break;
            case 4:
                esp_hidd_send_consumer_value(hid_conn_id, HID_CONSUMER_SCAN_NEXT_TRK, true);
                vTaskDelay(1);
                esp_hidd_send_consumer_value(hid_conn_id, HID_CONSUMER_SCAN_NEXT_TRK, false);
                break;
             case 10:
...
{   
    ...
    xTaskCreate(&touch_uer_task, "touch_task",1024, NULL, 5, NULL);
}